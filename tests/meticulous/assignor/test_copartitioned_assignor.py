...
def is_valid(
    cli_assignments: MutableMapping[str, CopartitionedAssignment],
    num_partitions: int,
    replicas: int,
) -> bool:
...
def clients_balanced(new: MutableMapping[str, CopartitionedAssignment]) -> bool:
    assert all(new[client].actives for client in new)
    return True


def client_addition_sticky(
...
def test_add_new_clients(partitions, replicas, num_clients, num_additional_clients):
    ...
    elif partitions and partitions >= num_clients + num_additional_clients:
...
def test_remove_clients(partitions, replicas, num_clients, num_removal_clients):
    assume(
        num_removal_clients < num_clients
        and replicas < (num_clients - num_removal_clients)
    )
    client_assignments = {
        str(client): CopartitionedAssignment(topics=_topics)
        for client in range(num_clients)
