...
def client_addition_balanced(
    new: MutableMapping[str, CopartitionedAssignment],
    num_clients: int,
    partitions: int,
) -> bool:
    if partitions >= num_clients and partitions >= len(new):
        assert all(new[client].actives for client in new if any(new[client].actives
                                                                for client in new))
    return True
...
def test_add_new_clients(partitions, replicas, num_clients, num_additional_clients):
    ...
    assert is_valid(new_assignments, partitions, replicas)
    # assert client_addition_balanced(new_assignments, num_clients, partitions)
