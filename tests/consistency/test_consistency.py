...
class Stresser(object):
    ...
    def _should(self):
        return (
            random.choices([True, False], [0.75, 0.25], k=1)[0]
            and not self._stop_stresser.is_set()
        )
    ...
    async def start(self, stopped_at_start=0):
        ...
        start_workers = random.sample(
            list(self.workers), self.num_workers - stopped_at_start
        )
        print(f"Start workers: {start_workers!r}")
        await asyncio.wait(
            [self._start_producer(producer) for producer in self.producers]
        )
        await asyncio.wait(
...
async def test_consistency(loop):
    ...
    checker = ConsistencyChecker(
        "withdrawals", "f-simple-user_to_total-changelog", loop=loop
    )
