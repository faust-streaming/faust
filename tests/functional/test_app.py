...
class Test_settings:
    ...
    def test_defaults(self):
        ...
        assert (
            conf.broker_rebalance_timeout == Settings.broker_rebalance_timeout.default
        )
        assert (
            conf.broker_heartbeat_interval == Settings.broker_heartbeat_interval.default
        )
        ...
        assert (
            conf.broker_commit_livelock_soft_timeout
        ...
        assert (
            conf.stream_processing_timeout == Settings.stream_processing_timeout.default
        )
        assert conf.producer_partitioner == Settings.producer_partitioner.default
        assert (
            conf.producer_request_timeout == Settings.producer_request_timeout.default
        )
        assert conf.producer_api_version == Settings.broker_api_version.default
        assert conf.producer_linger == 0.0
        assert (
            conf.stream_publish_on_commit == Settings.stream_publish_on_commit.default
        )
        assert conf.stream_wait_empty
        assert conf.broker_max_poll_records == Settings.broker_max_poll_records.default
        assert (
            conf.broker_max_poll_interval == Settings.broker_max_poll_interval.default
        )
        assert (
            conf.consumer_auto_offset_reset
