...
class Record(Model, abstract=True):  # type: ignore
    ...
    # The body of this method is code generated by ``_BUILD_asdict`` and
    # installed on every concrete Record subclass, so the empty body here is
    # deliberate: it only documents the signature.
    def asdict(self) -> Dict[str, Any]:  # type: ignore[empty-body] # pragma: no cover
